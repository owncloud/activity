        ...
        for suite, alternateSuiteName in suites.items():
            ...
            for testConfig in buildTestConfig(params):
                ...
                if isWebUI or isAPI or isCLI:
                    ...
                    name = "%s%s%s-%s%s-%s-p%s%s" % (alternateSuiteName, keyString, partString, testConfig["server"].replace("daily-", "").replace("-qa", ""), browserString, testConfig["database"].replace(":", ""), phpVersionForPipelineName, esString)
                    maxLength = 50
                    nameLength = len(name)
                    if nameLength > maxLength:
