OC.L10N.register(
    "activity",
    {
    "Activities" : "Aktivitäten",
    "Error loading activities" : "Fehler beim Laden der Aktivitäten",
    "No activities" : "Keine Aktivitäten",
    "Load more activities" : "Weitere Aktivitäten laden",
    ", " : ", ",
    "{parameterList} and {lastParameter}" : "{parameterList} und {lastParameter}",
    "_{parameterList} and {linkStart}%n more{linkEnd}_::_{parameterList} and {linkStart}%n more{linkEnd}_" : ["{parameterList} und {linkStart}%n mehr{linkEnd}","{parameterList} und {linkStart}%n mehr{linkEnd}"],
    "in {directory}" : "in {directory}",
    "This stream will show events like additions, changes & shares" : "Dieser Stream zeigt Ereignisse wie Hinzugekommenes, Änderungen und Freigaben an",
    "There are no events for this filter" : "Keine Ereignisse für diesen Filter vorhanden",
    "Today" : "Heute",
    "Yesterday" : "Gestern",
    "Personal activity feed for %s" : "Persönlicher Feed der Aktivitäten für %s",
    "Your feed URL is invalid" : "Deine Feed-URL ist ungültig",
    "Your settings have been updated." : "Deine Einstellungen wurden aktualisiert.",
    "Mail" : "Mail",
    "Stream" : "Stream",
    "\"remote user\"" : "„Entfernter Benutzer“",
    "Activity notification" : "Benachrichtigung über die Aktivitäten",
    "All Activities" : "Alle Aktivitäten",
    "Activities by you" : "Aktivitäten von Dir",
    "Activities by others" : "Aktivitäten von Anderen",
    "%s and %s" : "%s und %s",
    "Hello %s," : "Hallo %s,",
    "You are receiving this email because the following things happened at %s" : "Du erhältst diese E-Mail, da folgende Ereignisse auf %s geschehen sind",
    "* %1$s - %2$s" : "* %1$s - %2$s",
    "_* and %n more _::_* and %n more _" : ["* und %n weiterer","* und %n weitere"],
    "You are receiving this email because the following things happened at <a href=\"%s\">%s</a>" : "Du erhälst diese E-Mail weil Folgendes um  <a href=\"%s\">%s</a> passiert ist.",
    "%1$s - %2$s" : "%1$s - %2$s",
    "_and %n more _::_and %n more _" : ["und%n mehr","und %n mehr"],
    "Activity" : "Aktivität",
    "List your own file actions in the stream" : "Deine eigenen Datei-Aktivitäten im Stream auflisten",
    "Notify about your own actions via email" : "Über Deine eigenen Aktivitäten via E-Mail benachrichtigen",
    "You need to set up your email address before you can receive notification emails." : "Bevor Du E-Mail-Benachrichtigungen empfangen kannst, musst Du Deine E-Mail-Adresse einrichten.",
    "Send emails:" : "Sende E-Mails:",
    "Hourly" : "Stündlich",
    "Daily" : "Täglich",
    "Weekly" : "Wöchentlich",
    "Activity feed" : "Feed der Aktivitäten",
    "Enable RSS feed" : "RSS-Feed aktivieren",
    "No activity yet" : "Bislang keine Aktivitäten",
    "No more events to load" : "Keine weiteren Ereignisse zum Laden",
    "Files" : "Dateien"
},
"nplurals=2; plural=(n != 1);");
