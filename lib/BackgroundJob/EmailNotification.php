		...
		foreach ($affectedUsers as $user) {
			...
			try {
				...
				if ($this->mqHandler->validateMailAddress($email)) {
					$this->mqHandler->sendEmailToUser($user, $email, $language, $timezone, $sendTime);
				} else {
					$this->logger->debug("Couldn't send notification email to user '$user' due to non-RFC compliant address '$email'", ['app' => 'activity']);
				}
				$processedUsers[] = $user;
			} catch (\Exception $e) {
